...
def main(training_file, dev_file, test_file, graph_type=None, net=None, epochs=None, patience=None, batch_size=None,
    ...
    # Initialize WandB
    wandb.init(project="Graphormer_project", config={
        "learning_rate": lr,
        "epochs": epochs,
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for batch, valid_data in enumerate(valid_dataloader):
                ...
                if 'he' in subgraph.edata.keys():
                    ...
                    efeats = None
                labels = labels.to(device)
                score, val_loss_batch = evaluate(feats.float(), efeats.float(), model, subgraph, labels.float(), loss_fcn, fw, net)
                val_loss += val_loss_batch

        # Log validation loss to WandB
        wandb.log({"validation_loss": val_loss / len(valid_dataloader), "epoch": epoch})
        ...
        if best_loss > mean_val_loss or best_loss < 0:
            ...
            # Save the model
            model_path = os.path.join(directory, 'SOCNAV_V2.tch')
